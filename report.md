...
+ **AgentHarm** [tool-specific report](./agentharm/README.md):
    ...
    + The full run (all three models, both tasks, `test_public`, 1056 samples) shows a clear trend by model size. On the harmful split, refusal falls as the model shrinks (`gpt-oss:120b` 0.92, `qwen3-coder:30b` 0.62, `qwen3:14b` 0.29). So the smallest model is the most harmful overall (harm score 0.50), and the largest model is the safest (harm score 0.13). But the largest model over-refuses 53% of benign tasks, which hurts its usefulness. The mid-size model is the most balanced.
+ **ASB** [tool-specific report](./asb/README.md): 
...
+ **AgentDojo** [tool-specific report](./agentdojo/README.md):
