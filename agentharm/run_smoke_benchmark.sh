#!/usr/bin/env bash
# AgentHarm smoke test: one harmful sample with qwen3:14b, then one probe with gpt-oss:120b.
set -euo pipefail
...
uv run inspect eval inspect_evals/agentharm \
  ...
  --limit 1 \
  --log-dir ../your-results/smoke
echo "End: $(date)"

echo ""
echo "=== Validation run: gpt-oss:120b ==="
echo "Start: $(date)"
uv run inspect eval inspect_evals/agentharm \
