...
> [!NOTE]
> The install does not require Docker, a virtual machine, or a web server. The agent tools call simulated environment data (in-memory agent state with fixed response strings). Two code fixes are needed (conda fallback and judge model). The ChromaDB workaround applies only to non-memory attack runs.
...
> [!NOTE]
> The 5 attack types are defined in the `attack_prompts` dictionary in `react_agent_attack.py`.
> The 10 agent scenarios match the 10 entries in `data/agent_task.jsonl`.
...
The tools are simulated. `SimulatedTool.run()` returns the `expected_achivement` string (upstream typo for "achievement"). `AttackerTool.run()` returns a confirmation that the attack goal was reached.

> [!NOTE]
> The 4 attack mode flags correspond to the 4 injection surfaces. The `--direct_prompt_injection` flag activates DPI, `--observation_prompt_injection` activates OPI, and `--memory_attack` activates memory poisoning. PoT uses its own task file (`agent_task_pot.jsonl`).

### Scoring

- `ASR` (Attack Success Rate): did the agent follow the injected instruction? Checked by a substring match: the attack goal string must appear in any agent message.
- `Original Task Success Rate`: did the agent complete the legitimate task? Checked by looking for the legitimate tools' expected achievement strings in the messages.
- `Refusal Rate`: did the agent refuse the request? Checked by a judge model that reads the agent's messages and answers "1" (compliant) or "0" (refused).
...
> [!NOTE]
> Key dependency problems found during setup: (1) requirements.txt is missing 6 packages, (2) `conda list` call crashes without conda, (3) `openai==1.20.0` conflicts with langchain-openai, (4) `protobuf==4.25.3` conflicts with google-ai-generativelanguage.
...
> [!NOTE]
> `SimulatedTool.run()` returns the `expected_achivement` string. `AttackerTool.run()` returns a confirmation of the attack goal. Both are defined in the tool JSONL files under `data/`.
